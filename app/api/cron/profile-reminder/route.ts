...
const MIN_HOURS_BETWEEN_REMINDERS = 168 // 7 zile // nu trimitem mai des de 1x/24h
...
export async function GET(req: NextRequest) {
  ...
  const { data: influencers, error } = await admin
    .from('influencers')
    .select('id, user_id, name, bio, niches, avatar, city, platforms, slug, identity_verified, last_reminder_sent_at, email_reminders_enabled')
