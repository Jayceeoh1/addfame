...
export async function POST(req: NextRequest) {
  try {
    ...
    if (influencer) {
      ...
      // Notificare influencer că a primit un review
      if (influencer.user_id && reviewer_role === 'brand') {
        const stars = '⭐'.repeat(rating)
        await admin.from('notifications').insert({
          user_id: influencer.user_id,
          title: `${stars} Ai primit un review!`,
          body: comment
            ? `Un brand ți-a acordat ${rating}/5 stele: "${comment.substring(0, 80)}${comment.length > 80 ? '...' : ''}"`
            : `Un brand ți-a acordat ${rating}/5 stele pentru colaborarea finalizată.`,
