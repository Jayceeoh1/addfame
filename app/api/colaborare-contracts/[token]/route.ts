...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  try {
    ...
    const { data, error } = await admin()
      ...
      .select('id, token, influencer_name, influencer_email, influencer_phone, influencer_address, amount_lei, payment_days, campaign_title, platform, deliverables, status, signed_at, created_at')
      .eq('token', token)
      .single()
