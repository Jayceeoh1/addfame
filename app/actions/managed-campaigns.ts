...
export async function assignInfluencersToManaged(
  ...
  try {
    ...
    const { data: campaign } = await admin
      .from('campaigns').select('id, brand_id, title, brand_name, max_influencers, budget, campaign_type')
    ...
    // Activează campania și actualizează budget_per_influencer cu suma reală per influencer
    const actualPerInfluencer = influencersWithAmounts && influencersWithAmounts.length > 0
      ? Math.round(influencersWithAmounts.reduce((sum, { amount }) => sum + amount, 0) / influencersWithAmounts.length)
      : null

    await admin.from('campaigns')
      .update({
        status: 'ACTIVE',
        invited_influencers: influencerIds,
        ...(actualPerInfluencer ? { budget_per_influencer: actualPerInfluencer } : {}),
      })
      .eq('id', campaignId)
...
// ── Admin: invită influenceri la campanie BARTER ──────────────────────────────
export async function inviteInfluencersToBarter(
  campaignId: string,
  influencerIds: string[]
) {
  try {
    ...
    const { data: campaign } = await admin
      ...
      .eq('id', campaignId).eq('campaign_type', 'BARTER').single()
    if (!campaign) throw new Error('Campania barter nu există.')

    // Verifică care influenceri NU au deja o colaborare activă
    ...
    const existingMap = new Map((existing || []).map((e: any) => [e.influencer_id, e.status]))

    // Includem DOAR influencerii care nu au deja o colaborare
    // (nu suprascriEM pe cei deja PENDING/ACTIVE/COMPLETED)
    const toInsert = influencerIds
    ...
    const actualInvited = toInsert.length
    const skipped = influencerIds.length - actualInvited
    ...
    // Trimite emailuri DOAR celor nou invitați (nu celor care erau deja în campanie)
    try {
      const { emailInfluencerInvited } = await import('@/lib/email')
      for (const infId of toInsert.map((c: any) => c.influencer_id)) {
        const { data: inf } = await admin
    ...
    } catch (e) { console.error('[Barter invite emails failed]', e) }

    revalidatePath('/admin/campaigns')
    return { success: true, count: actualInvited, skipped }
  } catch (err: any) {
    return { success: false, error: err.message }
  }
}
