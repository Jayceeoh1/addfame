...
export default function InfluencerVerifyPage() {
  ...
  async function toBase64(file) {
    // Comprimăm imaginea înainte de a o converti în base64
    // ca să nu depășim limita de 1MB per row din Supabase
    return new Promise((resolve, reject) => {
      if (!file.type.startsWith('image/')) {
        // PDF — citim direct dar verificăm size
        if (file.size > 800 * 1024) {
          reject(new Error('Documentul PDF este prea mare. Te rugăm să îl comprimi sub 800KB.'))
          return
        }
        const reader = new FileReader()
      ...
      // Imagine — comprimăm și redimensionăm
      const img = new Image()
      ...
      img.onload = () => {
        ...
        // Calitate 0.7 — bun echilibru între dimensiune și claritate
