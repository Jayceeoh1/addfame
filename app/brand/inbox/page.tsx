...
function InboxContent() {
  ...
  return (
    <div className="flex h-[calc(100vh-64px)]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      {/* Sidebar */}
      <div className={`${selected ? 'hidden md:flex' : 'flex'} w-full md:w-72 flex-shrink-0 bg-white flex-col`} style={{ borderRight: '1.5px solid #f0f0f0' }}>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div key={group.campaignId}>
              <div className="px-4 pt-3 pb-1.5 sticky top-0 bg-white/95 backdrop-blur z-10">
                <p className="text-[10px] font-black uppercase tracking-wider text-orange-500 truncate">{group.campaignTitle}</p>
              </div>
              {group.items.map(c => (
      ...
      {!selected ? (
        <div className="hidden md:flex flex-1 items-center justify-center bg-gray-50">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">Alege un influencer căruia să îi scrii</p>
          </div>
        </div>
      ) : (
        <div className="flex-1 flex flex-col bg-gray-50 min-w-0">
          <div className="bg-white px-4 md:px-6 py-4 flex items-center gap-3 flex-shrink-0" style={{ borderBottom: '1.5px solid #f0f0f0' }}>
            {/* Back button pe mobile */}
            <button className="md:hidden w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0"
