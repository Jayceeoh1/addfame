...
import { getAllBrands, deleteBrand, approveBrandVerification, rejectBrandVerification, adminAdjustCredits, adminImpersonateUser } from '@/app/actions/admin'
import {
  Building2, Search, Trash2, RefreshCw, Shield, Clock,
  CheckCircle, XCircle, AlertCircle, ChevronDown, ExternalLink,
  FileText, Globe, Linkedin, X, Mail, Send, Smartphone} from 'lucide-react'
...
export default function AdminBrands() {
  ...
  const openEmailModal = (brand: any) => {
    setEmailSubject('Bună ziua din partea echipei AddFame 👋')
    setEmailBody(`Bună ziua,

Ne bucurăm că sunteți pe AddFame!

Suntem la doar 2 săptămâni de la lansare și am depășit deja 350 de influenceri înscriși — iar numărul crește în fiecare zi. Asta înseamnă că brandul dumneavoastră are acces la o comunitate activă și în plină expansiune.

Momentan lucrăm exclusiv pe campanii de tip barter — un model simplu și eficient: voi oferiți produse, influencerii creează conținut autentic. Fără costuri mari, fără contracte complicate.

Am vrea să vă invităm la un scurt call de 15-20 de minute în care:

• Vă arătăm cum funcționează o campanie barter de la A la Z
• Vă ajutăm să identificați influencerii potriviți pentru brandul vostru
• Răspundem la orice întrebare aveți

Ne puteți contacta oricând:
📞 +40 724 796 883
📧 ciprian@addfame.ro

Suntem flexibili și ne adaptăm după programul dumneavoastră. Ne-ar face plăcere să construim ceva frumos împreună!

Cu drag,
Ciprian Stancu
Echipa AddFame
ciprian@addfame.ro | +40 724 796 883 | addfame.ro`)
    setEmailModal(true)
  }

  const sendEmail = async () => {
    if (!selected?.email || !emailSubject.trim() || !emailBody.trim()) return
    ...
    try {
      ...
      notify('✅ Email trimis către ' + selected.email)
      setEmailModal(false)
    } catch (e: any) {
    ...
    <div className="p-6 lg:p-8 max-w-7xl mx-auto" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 flex justify-end" onClick={() => setSelected(null)}>
          <div className="w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="p-6">
              ...
              {/* CUI / CIF + date firma */}
              <div className="mb-5 p-4 rounded-2xl border border-gray-100 bg-gray-50 space-y-3">
                <div>
                  <p className="text-xs font-black text-gray-400 uppercase tracking-wider mb-1.5">CUI / CIF Firmă</p>
                  <div className="flex items-center gap-2">
                    <p className={`text-sm font-black ${selected.cui ? 'text-gray-900' : 'text-red-500'}`}>
                      {selected.cui || '⚠️ Nedeclarat'}
                    </p>
                    {selected.cui && (
                      <a
                ...
                <div>
                  <p className="text-xs font-black text-gray-400 uppercase tracking-wider mb-1.5">Denumire legală</p>
                  <p className={`text-sm font-bold ${selected.company_legal_name ? 'text-gray-900' : 'text-red-500'}`}>
                    {selected.company_legal_name || '⚠️ Nedeclarat'}
                  </p>
                </div>
                <div>
                  <p className="text-xs font-black text-gray-400 uppercase tracking-wider mb-1.5">Sediu social</p>
                  <p className={`text-sm font-bold ${selected.company_address ? 'text-gray-900' : 'text-red-500'}`}>
                    {selected.company_address || '⚠️ Nedeclarat'}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2 mb-5">
                {selected.phone && (
                  <a href={`tel:${selected.phone}`} className="flex items-center gap-3 p-3 rounded-xl bg-green-50 hover:bg-green-100 transition">
                    <Smartphone className="w-4 h-4 text-green-500 flex-shrink-0" />
                    <span className="text-sm font-bold text-green-700 flex-1">{selected.phone}</span>
                    <span className="text-xs text-green-500 font-bold">Sună</span>
                  </a>
                )}
                {selected.website && (
              ...
              {/* ── EMAIL CALL INVITATION ── */}
              <div className="mt-4 pt-4" style={{ borderTop: '1.5px dashed #f0f0f0' }}>
                <button
                  onClick={() => openEmailModal(selected)}
                  className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl font-black text-sm text-white transition"
                  style={{ background: 'linear-gradient(135deg,#f97316,#ec4899)', boxShadow: '0 3px 12px rgba(249,115,22,0.3)' }}
                >
                  <Mail className="w-4 h-4" /> Trimite Email Invitație Call
                </button>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead style={{ borderBottom: '1.5px solid #f0f0f0', background: '#fafafa' }}>
                <tr>
                  {['Brand', 'Email', 'Telefon', 'Industry', 'Credits', 'Verification', 'Actions'].map(h => (
                ...
                {visible.map(b => {
                  ...
                  return (
                    <tr key={b.id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                      ...
                      <td className="px-5 py-4 text-xs text-gray-500">{b.phone ? <a href={`tel:${b.phone}`} style={{color:'#16a34a',fontWeight:700}}>{b.phone}</a> : '—'}</td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5">
                          ...
                          {b.user_id && (
                            <button onClick={() => handleImpersonate(b.user_id, `brandul ${b.name}`)}
                              className="text-xs font-bold text-purple-500 hover:text-purple-700 transition px-2 py-1 rounded-lg hover:bg-purple-50"
      ...
      {emailModal && selected && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden" style={{ fontFamily: "inherit" }}>
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="font-black text-base text-gray-900">Trimite Email</h2>
                  <p className="text-xs text-gray-400">{selected.email}</p>
                </div>
              </div>
              <button onClick={() => setEmailModal(false)} className="w-8 h-8 rounded-full hover:bg-gray-100 flex items-center justify-center text-gray-400">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-wider mb-1.5">Subiect</label>
                <input
                  type="text"
                  value={emailSubject}
              ...
              <div>
                <label className="block text-xs font-black text-gray-500 uppercase tracking-wider mb-1.5">Mesaj</label>
                <textarea
                  value={emailBody}
                  onChange={e => setEmailBody(e.target.value)}
                  rows={12}
                  className="w-full px-3 py-2.5 border-2 border-gray-200 rounded-xl text-sm outline-none focus:border-orange-400 transition resize-none"
                  style={{ fontFamily: 'inherit' }}
                />
              </div>
              <div className="flex gap-3">
